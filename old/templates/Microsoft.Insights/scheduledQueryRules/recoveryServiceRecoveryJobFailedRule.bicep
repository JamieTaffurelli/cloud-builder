...
var queryName_var = '${vaultName}-recovery-failed-alert'
var query = 'AzureDiagnostics | where Category == "AzureSiteRecoveryJobs" and ResultType == "Failed" and Resource == "${toUpper(vaultName)}"'
...
resource queryName 'Microsoft.Insights/scheduledQueryRules@2018-04-16' = {
  ...
  tags: tags
  properties: {
    description: 'Query for Recovery Services Vault ${vaultName} recovery job status'
    enabled: enabled
    source: {
      query: query
