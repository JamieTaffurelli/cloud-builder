...
var queryName_var = '${vaultName}-failed-backup-alert'
var query = 'AzureDiagnostics | where Category == "AzureBackupReport" and OperationName == "Job" and JobOperation_s == "Backup" and JobStatus_s == "Failed" and Resource == "${toUpper(vaultName)}"'
...
resource queryName 'Microsoft.Insights/scheduledQueryRules@2018-04-16' = {
  ...
  tags: tags
  properties: {
    description: 'Query for Recovery Services Vault ${vaultName} backup job failure'
    enabled: enabled
    source: {
      query: query
