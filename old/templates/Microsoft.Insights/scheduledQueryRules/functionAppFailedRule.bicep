@description('The name of the Function App to pull data from')
...
var queryName_var = '${functionAppName}-failed-rule'
var query = 'FunctionAppLogs | where _ResourceId contains "${toUpper(functionAppName)}" and Level == "Error"'
...
resource queryName 'Microsoft.Insights/scheduledQueryRules@2018-04-16' = {
  ...
  tags: tags
  properties: {
    description: 'Query for Function App ${functionAppName} failure logs'
    enabled: enabled
    source: {
      query: query
