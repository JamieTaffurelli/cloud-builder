...
var queryName_var = '${runbookName}-failure-alert'
var query = 'AzureDiagnostics | where Category == "JobLogs" and Resource == "${toUpper(automationAccountName)}" and RunbookName_s == "${runbookName}" and (ResultType == "Failed" or ResultType == "Suspended")'
...
resource queryName 'Microsoft.Insights/scheduledQueryRules@2018-04-16' = {
  ...
  tags: tags
  properties: {
    description: 'Query for Automation Account ${automationAccountName} Runbook ${runbookName} failures'
    enabled: enabled
    source: {
      query: query
