...
var queryName_var = '${vaultName}-backup-alert-triggered'
var query = 'AzureDiagnostics | where Category == "AddonAzureBackupAlerts" and Resource == "${toUpper(vaultName)}"'
...
resource queryName 'Microsoft.Insights/scheduledQueryRules@2018-04-16' = {
  ...
  tags: tags
  properties: {
    description: 'Query for Recovery Services Vault ${vaultName} alerts triggered'
    enabled: enabled
    source: {
      query: query
