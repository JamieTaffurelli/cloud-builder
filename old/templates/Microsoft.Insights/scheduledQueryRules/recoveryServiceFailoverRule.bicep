...
var queryName_var = '${vaultName}-failover-alert'
var query = 'AzureDiagnostics | where Category == "AzureSiteRecoveryReplicatedItems" and (failoverHealth_s == "Critical" or failoverHealth_s == "Warning") and Resource == "${toUpper(vaultName)}"'
...
resource queryName 'Microsoft.Insights/scheduledQueryRules@2018-04-16' = {
  ...
  tags: tags
  properties: {
    description: 'Query for Recovery Services Vault ${vaultName} failover health status'
    enabled: enabled
    source: {
      query: query
