...
Describe "$(Split-Path -Path $PSCommandPath -Leaf)" {
    ...
    Context "Parameter Validation" {
        ...
        It "Errors if Path does not exist" {
            ...
            { "TestDrive:\file.json" | Set-AzBuildTemplateFilePathWithVersion -ErrorAction Stop } | should throw 'Cannot validate argument on parameter 'Path'. The " ($PSItem | Test-Path -PathType "Leaf") -and ([System.IO.Path]::GetExtension($PSItem) -eq "json") " validation script for the argument with value "TestDrive:\file.json" did not return a result of True. Determine why the validation script failed, and then try the command again.'
        }
        ...
        It "Errors if Path is not a JSON file" {
            ...
            { "TestDrive:\template.txt" | Set-AzBuildTemplateFilePathWithVersion -ErrorAction Stop } | should throw 'Cannot validate argument on parameter 'Path'. The " ($PSItem | Test-Path -PathType "Leaf") -and ([System.IO.Path]::GetExtension($PSItem) -eq "json") " validation script for the argument with value "TestDrive:\file.json" did not return a result of True. Determine why the validation script failed, and then try the command again.'
        }
    }
    ...
    Context "Version Validation" {
        ...
        It "Errors if contentVersion is not valid" {
            ...
            { "TestDrive:\template.json" | Set-AzBuildTemplateFilePathWithVersion -ErrorAction Stop } | should throw "TestDrive:\template.json does not have a valid content version ''"

            Mock -CommandName Get-Content -MockWith {'{ "contentVersion": "invalid"}'} -Verifiable

            { "TestDrive:\template.json" | Set-AzBuildTemplateFilePathWithVersion -ErrorAction Stop } | should throw "TestDrive:\template.json does not have a valid content version 'invalid'"
        ...
        It "Returns versioned file path" {
            ...
            "TestDrive:\template.json" | Set-AzBuildTemplateFilePathWithVersion -ErrorAction Stop | should be "TestDrive:\template.1.0.0.0.json"
