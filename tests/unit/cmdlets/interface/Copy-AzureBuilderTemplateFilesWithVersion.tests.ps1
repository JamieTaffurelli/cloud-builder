...
Describe "$(Split-Path -Path $PSCommandPath -Leaf)" {
    ...
    Context "Template Output Validation" {
        ...
        It "Warns if no template files were found" {
            ...
            Copy-AzureBuilderTemplateFilesWithVersion -SearchFolder "TestDrive:\templates" -OutputFolder "TestDrive:\versioned" | should be $null
