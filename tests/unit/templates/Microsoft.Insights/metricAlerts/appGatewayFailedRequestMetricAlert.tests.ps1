...
Describe "App Gateway Failed Request Metric Alert Parameter Validation" {
...
Describe "App Gateway Failed Request Metric Alert Resource Validation" {

    $failedRequest = $json.resources | Where-Object { $PSItem.properties.criteria.allof.metricName -eq "FailedRequests" }
    
    Context "type Validation" {

        It "alert type value is Microsoft.Insights/metricAlerts" {

            $failedRequest.type | should be "Microsoft.Insights/metricAlerts"
        }
    }
    ...
    Context "apiVersion Validation" {
        ...
        It "alert apiVersion value is 2018-03-01" {
            ...
            $failedRequest.apiVersion | should be "2018-03-01"
        }
    }
    ...
    Context "location Validation" {
        ...
        It "location value is global" {
            ...
            $failedRequest.location | should be "global"
        }
    }

    Context "metricName Validation" {

        It "metricName value is FailedRequests" {

            $failedRequest.properties.criteria.allof.metricName | should be "FailedRequests"
        }
    }

    Context "operator Validation" {

        It "operator value is global" {

            $failedRequest.properties.criteria.allof.operator | should be "GreaterThan"
        }
    }
    ...
    Context "timeAggregation Validation" {
        ...
        It "timeAggregation value is Total" {
            ...
            $failedRequest.properties.criteria.allof.timeAggregation | should be "Total"
        }
    }
}
...
Describe "Metric Alert Output Validation" {
    ...
    $failedRequest = $json.outputs.metricAlert

    Context "Failed Request Metric Alert Reference Validation" {

        It "type value is object" {

            $failedRequest.type | should be "object"
        }

        It "Uses full reference for Metric Alert" {

            $failedRequest.value | should be "[reference(resourceId('Microsoft.Insights/metricAlerts', variables('appGatewayFailedRequestAlertName')), '2018-03-01', 'Full')]"
        }
    }
}