...
Describe "Public Certificate Parameter Validation" {
    ...
    Context "slotName Validation" {
        ...
        It "slotName parameter is mandatory" {
            ...
            ($json.parameters.slotName.PSObject.Properties.Name -contains "defaultValue") | should be $false
        }
    }
...
Describe "Public Certificate Resource Validation" {
    ...
    Context "type Validation" {
        ...
        It "type value is Microsoft.Web/sites/slots/publicCertificates" {

            $json.resources.type | should be "Microsoft.Web/sites/slots/publicCertificates"
        }
    }
...
Describe "Public Certificate Output Validation" {
    ...
    Context "Public Certificate Reference Validation" {
        ...
        It "Uses full reference for Public Certificate" {

            $json.outputs.cert.value | should be "[reference(resourceId('Microsoft.Web/sites/slots/publicCertificates', parameters('appName'), parameters('slotName'), parameters('certificateName')), '2016-03-01', 'Full')]"
        }
    }
}