...
Describe "Authorization Rule Resource Validation" {
    ...
    $authorizationRule = $json.resources | Where-Object { $PSItem.type -eq "Microsoft.ServiceBus/namespaces/authorizationRules" }

    Context "type Validation" {

        It "type value is Microsoft.ServiceBus/namespaces/authorizationRules" {

            $authorizationRule.type | should be "Microsoft.ServiceBus/namespaces/authorizationRules"
        }
    }
...
Describe "Authorization Rule Output Validation" {
    ...
    Context "Authorization Rule Reference Validation" {
        ...
        It "Uses full reference for Authorization Rule" {
            ...
            $json.outputs.authorizationRule.value | should be "[reference(resourceId('Microsoft.ServiceBus/namespaces/authorizationRules', parameters('serviceBusName'), parameters('authorizationRuleName')), '2018-01-01-preview', 'Full')]"
        }
    }
}